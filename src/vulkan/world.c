...
struct graphics_world graphics_load_sw(
    ...
    for (uint32_t i = 0; i < VK_MAX_OBJECTS && i < SW_MAX_OBJECTS
        ...
        gworld.objects[i].texture = &gworld.textures[world.objects[i].texture_index];
        gworld.objects[i].flags = VK_FLAG_EXISTS;
        tf_set(&gworld.objects[i].transform, world.objects[i].position, world.objects[i].rotation,
            world.objects[i].scale);
    }
